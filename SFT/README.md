...
- Trainer：负责基于 `TRL` 的 `SFTTrainer` / 后续 `DPOTrainer` 进行模型加载、LoRA/QLoRA 配置、训练、验证和训练产物保存
...
当前训练与下载流程统一使用 `configs/train_config.yaml` 中的以下字段：

- `model.model_id`：远程模型标识，例如 `Qwen/Qwen3-0.6B`
- `model.local_model_dir`：本地基础模型目录，例如 `model/base/Qwen3-0.6B`
- `run.output_dir`：训练产物目录，默认放在 `trained/`

行为约定如下：

- 独立下载脚本与训练脚本共用同一份训练配置
- 手动下载时，使用 `modelscope` 将 `model.model_id` 下载到 `model.local_model_dir`
- 训练时优先从 `model.local_model_dir` 加载基础模型
- 如果训练启动时发现本地目录不存在，或目录下缺少模型配置和权重文件，则自动触发下载
- 训练完成后，adapter、tokenizer 和运行信息统一保存到 `run.output_dir`
...
- `openai_compat`：通过兼容 OpenAI Chat Completions 的远程 API 进行推理
- `local_hf`：通过本地 Hugging Face 目录推理

当使用 `local_hf` 时：

- `backend.base_model_path` 应指向本地基础模型目录，例如 `model/base/Qwen3-0.6B`
- `backend.adapter_path` 可选，指向训练输出目录，例如 `trained/qwen3_0_6b_lora`
- 如果只填 `base_model_path`，则走底座模型原始推理
- 如果同时填写 `base_model_path + adapter_path`，则在底座模型上加载 LoRA adapter
...
- `configs/`：放训练配置
- `model/base/`：放基础预训练模型本地副本
- `trained/`：放训练 adapter、tokenizer、日志和运行 manifest
...
- `m1`：清洗并整理 Extracter 样本，输出 `prepared_samples.jsonl`
- `m2`：构造 chat-format 训练数据并切分 `train / val / test`
...
- 两个入口都读取同一份训练配置
- 实际下载模型由 `model.model_id` 决定
- 本地落盘目录由 `model.local_model_dir` 决定
...
- 训练优先从 `model.local_model_dir` 加载基础模型
- 如果本地模型不存在，会先自动下载
- 训练产物统一写入 `run.output_dir`
...
- 使用 `openai_compat` 时，需要在配置或环境变量中提供 API Key
- 使用 `local_hf` 时，需要在推理配置中填写 `base_model_path`，可选填写 `adapter_path`
...
- 默认评估输入由 `default_eval_input_path` 指定
- 评估结果会输出到 `SFT/output/`
