...
推荐统一使用多轮 chat JSONL，与后续 DPO 阶段共用数据处理接口。单条样本形式如下：
...
- `system`：固化长期稳定的任务规则
- `user`：承载单样本的现象描述、字段约束和生成要求
- `assistant`：固定为结构化 JSON 字符串，不添加额外解释文字
...
2. 执行高质量过滤和重复去重
3. 按难度做样本分层（留出接口并在流程中调用，实际逻辑暂时不实现）
...
（数据清洗留出接口并在流程中调用，实际逻辑暂时不实现，可能后需要结合大语言模型实现）
...
（留出接口并在流程中调用，实际逻辑暂时不实现，可能后需要结合大语言模型实现，对应字段信息需要留空）
