...
def train(config: TrainConfig) -> None:
    ...
    with stage_progress(total=5, desc="M3 Training") as progress:
        ...
        progress.advance("tokenizer ready")
        ...
        progress.advance("datasets ready")
        ...
        progress.advance("trainer ready")

        trainer.train()
        progress.advance("training complete")

        trainer.save_model()
        tokenizer.save_pretrained(config.run.output_dir)
        progress.advance("artifacts saved")
